...
class Analise_de_dados:
    def __init__(self):
        self.__MRE = pd.read_excel(os.path.join('utils/analise_de_dados/dados', 'Mre.xlsx'))
        self.__SCE = pd.read_excel(os.path.join('utils/analise_de_dados/dados', 'Sce.xlsx'))
        self.__CIEE = pd.read_excel(os.path.join('utils/analise_de_dados/dados', 'Ciee.xlsx'))
        self.__EXIT_PATH = os.path.join(os.path.join(os.path.expanduser('~'), 'Downloads'), f"Resultado Analise de Dados {datetime.now().strftime('%Y_%m_%d')}.xlsx")
